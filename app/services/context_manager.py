...
class ContextManager:
    ...
    def __init__(self, storage: FileStorage, llm_client: LLMClient, config: ContextConfig):
    ...
    async def _build_system_prompt(
        ...
        # 章节信息（只需 title 和 summary，用元数据即可）
        if chapter_id:
            chapter = await self.storage.get_chapter(chapter_id)
            if chapter:
    ...
    async def _retrieve_characters(
        ...
        """检索与查询文本相关的角色"""
        all_chars = await self.storage.list_characters_by_project(project_id)
        if not all_chars:
            return []
    ...
    async def _retrieve_worldviews(
        ...
        """检索与查询文本相关的世界观条目"""
        all_wvs = await self.storage.list_worldviews_by_project(project_id)
        if not all_wvs:
            return []
    ...
    async def maybe_compress(
        ...
        # 插入摘要消息（is_summary=True，role=system）
        ...
        # 删除被压缩的原始消息
        await self.storage.delete_messages_by_ids(msg_ids)
        return True
    ...
    async def _count_messages(
        ...
        """统计非摘要消息数"""
        return await self.storage.count_messages(project_id, chapter_id)

    async def _get_oldest_messages(
        self, project_id: int, chapter_id: int | None, limit: int
    ) -> list[dict]:
        """获取最早的 N 条非摘要消息（按 id 正序）"""
        return await self.storage.get_oldest_messages(
    ...
    async def _get_latest_summary(
        ...
        """获取最近的摘要消息内容"""
        return await self.storage.get_latest_summary(project_id, chapter_id)
